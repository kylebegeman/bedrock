#!/usr/bin/env bash
# M7 on the lane, bedrock's half: an app shaped like Loom's Core. Secrets the
# manifest asks bedrock to make and derive; a database from another image
...
run 'bedrock integration list' | grep -q '^storage' || fail "run prove-m5.sh first: the backup needs the storage integration"
...
for n in BEDROCK_POSTGRES_PASSWORD MINIO_ROOT_USER MINIO_ROOT_PASSWORD NOTES_OWNER_PASSWORD NOTES_APP_PASSWORD NOTES_DATABASE_URL PGPASSWORD MC_HOST_LOCAL; do
...
run "docker exec bedrock-notes-postgres grep -c scram-sha-256 /var/lib/postgresql/data/pgdata/pg_hba.conf" | grep -qv '^0$' || fail "local connections don't ask for a password"
...
run 'bedrock run notes buckets -- ls LOCAL' | quiet | grep -q 'notes/' || fail "no notes bucket"
...
before=$(worker)
if ! out=$(run 'bedrock deploy /srv/lane/notes --yes' 2>&1 | quiet); then echo "$out"; fail "the second deploy failed"; fi
...
if out=$(run 'bedrock deploy /srv/lane/notes --yes' 2>&1 | quiet); then echo "$out"; fail "a worker that never becomes ready was deployed"; fi
...
run "sed -i \"s/insert into notes (body) values ('deployed')/select 1 \\/ 0/\" /srv/lane/notes/bedrock.yaml"
if out=$(run 'bedrock deploy /srv/lane/notes --yes' 2>&1 | quiet); then echo "$out"; fail "a failing release was deployed"; fi
echo "$out" | grep -E "division by zero|nothing new started|failed" | head -3
run "sed -i \"s/select 1 \\/ 0/insert into notes (body) values ('deployed')/\" /srv/lane/notes/bedrock.yaml"
[[ "$(worker)" == "$good_worker" && "$(active)" == "$good" ]] || fail "a failing release touched the running revision"
[[ $(run "docker ps -a --filter label=bedrock.app=notes --format '{{.Names}}'" | grep -vc "$good\|postgres\|objects" || true) == 0 ]] || fail "a failing release left containers behind"
...
got=$(run "bedrock run notes api --secret PGPASSWORD -- sh -c 'test -n \"\$PGPASSWORD\" && echo has-the-secret'" | quiet)
...
got=$(printf %s "$word" | run "bedrock run notes api --stdin -- sh -c 'read -r v; printf %s \"\$v\" | sha256sum'")
[[ "$got" == "$want"* ]] || fail "--stdin didn't reach the command: $got"
run "bedrock jobs notes api --json" | grep -q "$word" && fail "the standard input reached the job record"
code=0; run "bedrock run notes api --stdin -- sh -c 'exit 7'" </dev/null || code=$?
[[ $code == 7 ]] || fail "bedrock run --stdin exited $code, not the command's 7"
...
if ! out=$(run 'bedrock backup notes --yes' 2>&1 | quiet); then echo "$out"; fail "the backup failed"; fi
...
if ! out=$(run 'bedrock drill notes --yes' 2>&1 | quiet); then echo "$out"; fail "the drill failed"; fi
...
run 'bedrock exposure notes' | grep -E "notes (database|object store)" | head -4
run 'bedrock exposure notes' | grep -q "notes object store" || fail "bedrock exposure doesn't know the object store"

echo "== remove with its data, then restore from the bucket and deploy again"
run 'bedrock remove notes --data --yes' 2>&1 | quiet | grep -E "removed|stopped|forgotten" | head -12
[[ "$(run "docker volume ls -q | grep -c '^bedrock-notes-' || true")" == 0 ]] || fail "volumes outlived bedrock remove --data"
run 'test ! -e /var/lib/bedrock/apps/notes' || fail "the first-run scripts' copy outlived the app"
if ! out=$(run 'bedrock restore notes --yes' 2>&1 | quiet); then echo "$out"; fail "the restore failed"; fi
echo "$out" | grep -E "snapshot|volume|postgres .* ready|database restored|succeeded"
if ! out=$(run 'bedrock deploy /srv/lane/notes --yes' 2>&1 | quiet); then echo "$out"; fail "the deploy after the restore failed"; fi
...
run 'bedrock run notes buckets -- ls LOCAL' | quiet | grep -q 'notes/' || fail "the restored object store lost its bucket"
echo "-- 3 restored rows and the new deploy's, still notes_owner's, and the bucket is back"

echo "M7 proven on the lane (bedrock's half): a Core-shaped app deploys, fails safely, and comes back from its bucket"
