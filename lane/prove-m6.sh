...
# is pointed here; a GitHub-style webhook deploys a push; bedrock deploy --to
...
trap 'run "bedrock git deny begamin \"$(cat "$push_key.pub")\"" >/dev/null 2>&1 || true; rm -rf "$work"' EXIT
...
echo "== build and install; reconcile keeps the bedrock user that receives pushes"
(cd "$here/.." && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -o bin/bedrock-linux-amd64 ./cmd/bedrock \
  ...
  && go build -trimpath -o bin/bedrock ./cmd/bedrock)
...
run 'chmod 755 /usr/local/bin/bedrock.new /srv/lane/bin/fakeflare.new && mv -f /usr/local/bin/bedrock.new /usr/local/bin/bedrock && mv -f /srv/lane/bin/fakeflare.new /srv/lane/bin/fakeflare && bedrock daemon install >/dev/null && bedrock version'
run 'bedrock host reconcile --yes' | quiet | grep -E "pushes|succeeded|failed"
run 'bedrock ps' | grep -q dragon-writer || fail "run prove-m4.sh and prove-m5.sh first"
...
after=$(run 'bedrock ps' | awk '$1=="begamin" {print $3}')
...
as_key() { ssh -i "$push_key" -o IdentityAgent=none -o IdentitiesOnly=yes -o BatchMode=yes -o UserKnownHostsFile="$here/known_hosts" "bedrock@$HOST" "$@" 2>&1 </dev/null | tail -1 || true; }
...
echo "== bedrock deploy --to sends this Mac's working tree, uncommitted change included"
echo "lane: sent without a commit" > "$work/begamin/LANE.md"
BEDROCK_SSH="$push_ssh" "$here/../bin/bedrock" deploy "$work/begamin" --to "bedrock@$HOST" 2>&1 | quiet | tail -3
...
serving() { docker ps --filter "label=bedrock.app=$1" --format '{{.Names}} {{.Label "bedrock.workload"}}' | awk '$2!="postgres" {print $1}' | grep -v -- '-job-' | head -1; }
containers() { docker ps --filter "label=bedrock.app=$1" --format '{{.Names}}' | grep -v -- '-job-'; }
...
for a in $apps; do
  ...
  edge_ok=closed
  for ip in $(ips bedrock-edge); do [ "$(probe "$from" "$ip" 443)" = open ] && edge_ok=open; done
  ...
  for ip in $(ips bedrock-edge); do
    tried=$((tried + 1))
    if [ "$(probe "$from" "$ip" 2019)" = open ]; then echo "REACHED: $a -> the edge's admin at $ip:2019"; bad=1; fi
  done
done
dw=$(serving dragon-writer)
[ "$(probe "$dw" bedrock-dragon-writer-postgres 5432)" = open ] || { echo "control: dragon-writer can't reach its own database"; bad=1; }
...
run 'umask 077; bedrock git webhook begamin --repo file:///srv/lane/upstream/begamin.git 2>/root/.lane-webhook.out; awk "/Secret/ {print \$2}" /root/.lane-webhook.out > /root/.lane-webhook-secret; grep -v Secret /root/.lane-webhook.out; rm -f /root/.lane-webhook.out'
...
until run 'bedrock git webhooks --json' | python3 -c "import json,sys; h=json.load(sys.stdin)[0]; sys.exit(0 if h.get('last_commit')=='${hooked:0:12}' and h.get('last_result') else 1)"; do
  (( SECONDS < deadline )) || fail "the webhook's deploy didn't finish in 5 minutes"
  sleep 5
done
run 'bedrock git webhooks'
run 'bedrock git webhooks --json' | python3 -c "import json,sys; h=json.load(sys.stdin)[0]; assert h['last_result']=='deployed', h" || fail "the webhook's deploy failed"
...
run 'bedrock git webhook begamin --off; rm -f /root/.lane-webhook-secret'

echo "== history"
run 'bedrock history --limit 14'
echo "M6 proven: git push deploys begamin and a failing push is refused; its records appear and disappear with it; no app reaches another's network or the edge's admin; a name at another machine waits for bedrock dns point; a webhook deploys a push; bedrock deploy --to sends a working tree"
